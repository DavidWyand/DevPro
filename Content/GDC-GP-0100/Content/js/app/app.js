...
app.run(function($rootScope, $window, $location, $state, $sce, $interval, $timeout, $log, $anchorScroll) {
	...
	$rootScope.launchUploader = function (path, e) {
		
		e.preventDefault();
		$('#myUploadModal').appendTo('body').modal('show');
		return false;

	};

	$rootScope.sendExercise = function (sourceFilePath,orgName,version,sourceName,course)
	{
	
		{
			...
			if( isProgramming )
			{
			    ...
			    xhr.open('POST', 'http://10.1.10.10:3000',true);
			    ...
			    //var form = document.getElementById('uploadwork');
				var formData = new FormData();
				console.log(document.getElementById("upload_File"));
				formData.append("myFile", document.getElementById("upload_File").files[0]);
				
				//console.log(document.getElementById('upload_File'));
			    xhr.send(formData);
			}else
			{
				alert("Uploading is only available in Computer Science in Game Programming (C#) at this time.");
			}
		}
	}
